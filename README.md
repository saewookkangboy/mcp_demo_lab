...
- 패널 하단에서 현재 프로젝트의 테스트 명령을 바로 복사할 수 있습니다.
...
6. 패널 하단의 테스트 명령으로 같은 규칙을 자동 검증합니다.
...
테스트 이름은 사용자의 기대 행동을 먼저 읽도록 작성했습니다. 각 프로젝트 README의 확장 실습은 `테스트 수정 → 실패 확인 → 로직 구현 → 통과 확인` 순서로 진행합니다.
