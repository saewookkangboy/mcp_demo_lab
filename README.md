...
- 실행 기록과 결과의 화면 검증
- 외부 MCP 연결 구조 시연
- 기본 실습 완료 후 Next Stage 커스텀 개발 미션 이동

## Next Stage — 커스텀 개발 미션

기본 5개 실습을 완료하면 `http://localhost:4173/next-stage.html`에서 기존 프로젝트를 확장하는 세 가지 미션을 선택할 수 있습니다.

| 미션 | 난이도 | 기준 프로젝트 | 새 개발 결과 |
|---|---|---|---|
| 견적 할인 정책 | 초급 | 02 견적서 작성 | 할인액·최종 합계·할인 근거 |
| 출입문과 안전 동선 | 중급 | 03 설계도 제작 | 출입문·피난 동선 SVG·경고 |
| 신뢰도 기반 메모리 충돌 | 고급 | 05 데이터 기반 메모리 | 우선순위·탈락 사건·결정 근거 |

모든 미션은 `실패 테스트 → MCP 도구 확장 → FE 연결 → Harness 비교 → 전체 검증 → 제출` 순서를 사용합니다. 화면의 `실습 프롬프트 복사`를 누르면 Claude Code 또는 Codex에서 바로 시작할 수 있습니다. 자세한 계약과 제출 기준은 [Next Stage 커스텀 개발 가이드](docs/NEXT_STAGE_CUSTOM_MISSIONS.md)를 참고하세요.
...
정상 입력에서는 업무 데이터가 같더라도 최종 전달 상태가 명확히 달라집니다. 미적용은 `PASS_THROUGH / UNVERIFIED / 증거 없음`, 적용은 `ALLOW / VERIFIED / 증거 ID`로 표시됩니다. `계약 위반 결과 비교`를 누르면 불완전 결과가 미적용 상태에서는 화면까지 전달되고, 적용 상태에서는 `BLOCK / REJECTED`로 차단되는 차이를 확인할 수 있습니다.
